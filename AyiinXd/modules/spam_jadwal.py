from AyiinUbot import CMD_HELP, cmd
...
• `{cmd}setlist <nama_list>\n<teks>`  
  └─› Simpan teks spam multi-line & hyperlink.

• `{cmd}setlistfw <nama_list> <link_post>`  
  └─› Simpan spam forward dari channel.

• `{cmd}setgc <nama_list> <@grup1> <@grup2>`  
  └─› Tambah grup target ke list.

• `{cmd}delgc <nama_list> <@grup>`  
  └─› Hapus satu grup dari list spam.

• `{cmd}dellist <nama_list>`  
  └─› Hapus list & semua grupnya.

• `{cmd}spamtime <hh:mm> <nama_list>`  
  └─› Spam sekali di jam tertentu (format 24 jam).

• `{cmd}spamhari <Hari,hh:mm> <nama_list>`  
  └─› Spam sekali di hari dan jam tertentu.

• `{cmd}spaminterval <menit> <nama_list>`  
  └─› Spam otomatis tiap x menit.

• `{cmd}viewspam`  
  └─› Cek semua spam yang sedang aktif.

• `{cmd}stoplist <nama_list>`  
  └─› Hentikan spam aktif dari list tersebut.
"""
})


Ini dia Cong, modul spamjadwal.py lengkap siap tempel buat userbot lu. Semua fitur udah support:

Spam teks biasa & forward dari channel

Penjadwalan jam, hari, interval

Pengaturan grup target

Cek & stop spam berjalan

Support hyperlink dan teks multi-line


Kalau butuh gua bantu install, testing, atau tambah fitur auto-delete spam juga bisa. Mau?
