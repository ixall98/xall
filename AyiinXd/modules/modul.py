...
async def on_plug_in_callback_query_handler(event):
    if event.query.user_id == uid or event.query.user_id in SUDO_USERS:
        ...
        cmdhel = str(CMD_HELP.get(modul_name, "Tidak ditemukan."))
        if len(cmdhel) > 950:
            help_string = (
                cmdhel.replace("`", "").replace("**", "")[:950]
                + "..."
                + "\n\nBaca Teks Berikutnya Ketik .help "
                + modul_name
            )
        else:
            help_string = cmdhel.replace("`", "").replace("**", "")

        await event.edit(
            help_string,
            buttons=[Button.inline("ʙᴀᴄᴋ", data=b"reopen")],
        )
    else:
        await event.answer(
